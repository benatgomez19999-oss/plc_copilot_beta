import { dedupDiagnostics, diag, sortDiagnostics, toArtifactDiagnostic, } from '@plccopilot/codegen-core';
 ...
 */
export function computeRockwellDiagnostics(program) {
    const out = [];
    out.push(diag('info', 'ROCKWELL_EXPERIMENTAL_BACKEND', 'Rockwell ST POC backend: textual artifacts only, not directly importable to Studio 5000', {
        hint: 'use the Siemens or Codesys backends for production output until the L5X exporter ships',
    }));
    out.push(diag('info', 'ROCKWELL_NO_L5X_EXPORT', 'Rockwell backend does not emit a Studio 5000 L5X archive — only `.st` text files', {
        hint: 'a future sprint will add an L5X writer with controller / program / routine wrapping',
    }));
    if (hasTimers(program)) {
        out.push(diag('warning', 'ROCKWELL_TIMER_PSEUDO_IEC', 'IEC TON instances are rendered with `.Q` semantics; Studio 5000 needs Logix TIMER tags with `.DN` / `.ACC`', {
            hint: 'a future L5X backend will rewrite TonCall → Logix TIMER instructions',
        }));
    }
    return out;
}
function hasTimers(program) {
 ...
 * not mutated.
 */
export function withRockwellDiagnostics(program) {
    const merged = sortDiagnostics(dedupDiagnostics([
        ...program.diagnostics,
 ...
 */
export function renderProgramArtifactsRockwell(program) {
...
function renderBlockArtifact(fb, program) {
    const path = `${ROCKWELL_DIR}/${fb.name}.st`;
    const content = renderFunctionBlockRockwell(fb);
    const stationDiags = fb.stationId
        ? program.diagnostics
            .filter((d) => d.stationId === fb.stationId)
            .map(toArtifactDiagnostic)
        : [];
    const artifact = { path, kind: 'st', content };
    if (stationDiags.length > 0)
        artifact.diagnostics = stationDiags;
    return artifact;
}

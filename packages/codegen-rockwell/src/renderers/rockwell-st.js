import { renderRef as renderRefCore, renderSymbol as renderSymbolCore, } from '@plccopilot/codegen-core';
 ...
 *   #name                → name
 */
export function siemensToRockwellText(s) {
    let r = s;
...
export function renderExprRockwell(e) {
    switch (e.kind) {
        case 'Raw':
...
// ---------- Statement rendering ----------
export function renderStmtRockwell(stmt, level) {
    switch (stmt.kind) {
        case 'Assign': {
            const c = stmt.comment ? ` // ${stmt.comment}` : '';
            return [
                pad(level, `${renderRef(stmt.target)} := ${renderExprRockwell(stmt.expr)};${c}`),
            ];
        }
        case 'Comment':
        ...
        case 'TonCall':
            ...
            return [
                pad(level, `${renderRef(stmt.instance)}(IN := ${renderExprRockwell(stmt.inExpr)}, PT := T#${stmt.ptMs}MS); // pseudo-IEC TON`),
            ];
        case 'FbCall': {
            if (stmt.instance.kind === 'fbInstance' &&
 ...
 */
function renderEdgeOneShot(stmt, level) {
    if (stmt.instance.kind !== 'fbInstance')
        return [];
...
function renderCaseRockwell(stmt, level) {
    const out = [];
    ...
    for (const arm of stmt.arms) {
        ...
        if (arm.body.length === 0) {
            ...
            for (const s of arm.body)
                out.push(...renderStmtRockwell(s, level + 2));
        }
    }
    out.push(pad(level, 'ELSE'));
 ...
 */
export function renderVarSectionRockwell(section, level) {
    const out = [pad(level, section.section)];
    for (const d of section.decls) {
        if (d.preComment)
            out.push(pad(level + 1, `// ${d.preComment}`));
        out.push(...renderVarDeclRockwell(d, level + 1));
    }
    out.push(pad(level, 'END_VAR'));
    return out;
}
function renderVarDeclRockwell(d, level) {
 ...
 */
export function renderFunctionBlockRockwell(fb) {
    const lines = [];
    lines.push(`(* ============================================================`);
    ...
    lines.push(` * ============================================================ *)`);
    for (const hc of fb.headerComments)
        lines.push(`// ${hc}`);
    lines.push('');
    ...
    for (const s of fb.body)
        lines.push(...renderStmtRockwell(s, 0));
    lines.push('');

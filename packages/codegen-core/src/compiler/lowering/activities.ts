import type { State, Station } from '@plccopilot/pir';
 ...
 * activation entry.
 */
export function lowerStateActivity(
  state: State,
  station: Station,
  ...
  if (a.on_entry && a.on_entry.length > 0) {
    out.push(
      ir.comment(
        `on_entry: ${a.on_entry.length} action(s) — TODO: edge-driven lowering`,
      ),
    );
  }
  ...
  if (a.activate) {
    a.activate.forEach((activationRef, activateIndex) => {
      const activatePath =
        pathContext && stateIndex !== undefined
      ...
      if (!parsed || !parsed.activity) {
        diagnostics.push(
          diag(
            'error',
            'INVALID_ACTIVATION',
            `Activation "${activationRef}" must be "equipmentId.activityName".`,
            {
      ...
      if (!eq) {
        diagnostics.push(
          diag(
            'error',
            'UNKNOWN_EQUIPMENT',
            `Activation "${activationRef}" refers to equipment "${parsed.equipment_id}" which is not in station "${station.id}".`,
            {
              ...(activatePath !== undefined ? { path: activatePath } : {}),
              stationId: station.id,
      ...
      out.push(
        ir.assign(
          ref.local(commandVarName(parsed.equipment_id, parsed.activity)),
          ir.boolLit(true),
        ),
      );
    });
  ...
  if (a.on_exit && a.on_exit.length > 0) {
    out.push(
      ir.comment(
        `on_exit: ${a.on_exit.length} action(s) — TODO: edge-driven lowering`,
      ),
    );
  }

import type { Station, Transition } from '@plccopilot/pir';
...
import type { CaseArmIR, ExprIR, StmtIR } from '../ir/nodes.js';
...
export function lowerWildcardTransitions(
  station: Station,
  stateIndex: Map<string, number>,
  table: SymbolTable,
  edges: EdgeRegistry,
...
function lowerTransition(
  ...
  if (toIdx === undefined) {
    ...
    diagnostics.push(
      diag(
        'error',
        'UNKNOWN_STATE',
        `Transition "${t.id}" targets unknown state "${t.to}".`,
        {
  ...
  const guardContext: ExpressionDiagnosticContext | undefined = pathContext
    ? {
        path: transitionGuardPath(
          pathContext.machineIndex,
          pathContext.stationIndex,
          transitionIndex,
        ),
    ...
    : undefined;
  const triggerContext: ExpressionDiagnosticContext | undefined = pathContext
    ? {
  ...
  const parts: ExprIR[] = [];
  if (t.trigger) {
    const { ir: triggerIr, diagnostics: triggerDiags } = lowerExpression(
      t.trigger,
      table,
      edges,
      triggerContext,
  ...
  if (t.guard) {
    const { ir: guardIr, diagnostics: guardDiags } = lowerExpression(
      t.guard,
      table,
      edges,
      guardContext,
  ...
  const cond: ExprIR =
    parts.length > 0 ? ir.andAll(parts) : ir.boolLit(true);

  const tag =
    `t=${t.id} prio=${t.priority}` +
    (t.timeout

...
import { dedupDiagnostics, diag, firstError, sortDiagnostics, toArtifactDiagnostic, } from '../diagnostics.js';
 ...
 */
export function resolveFeatures(requested, hasAlarms) {
    const r = requested ?? {};
...
export function compileProject(project, options) {
    ...
    if (!machine) {
        ...
        throw new CodegenError('NO_MACHINE', `Project "${project.id}" must contain at least one machine for codegen.`, {
            path: 'machines',
    ...
    for (let si = 0; si < machine.stations.length; si++) {
        ...
        for (let ei = 0; ei < s.equipment.length; ei++) {
            ...
            if (!SUPPORTED_TYPES.has(eq.type)) {
                ...
                throw new CodegenError('UNSUPPORTED_EQUIPMENT', `Equipment "${eq.id}" has type "${eq.type}" which is not supported by the core pipeline.`, {
    ...
    if (machine.alarms.length > 0 && !features.useDbAlarms) {
        programDiagnostics.push(diag('info', 'ALARMS_AS_LOOSE_TAGS', `feature useDbAlarms=false: station FBs write alarms as loose tags; DB_Alarms / FB_Alarms are NOT emitted`, {
    ...
    programDiagnostics.push(...collectAlarmDiagnostics(machine, { machineIndex: 0 }));
    ...
    const aggregatedDiagnostics = sortDiagnostics(dedupDiagnostics(programDiagnostics));
    ...
    const manifest = buildManifest(project, options, aggregatedDiagnostics, features);
    return {
...
function collectDataBlocks(project, features) {
    const out = [];
 ...
 */
function buildManifest(project, options, diagnostics, features) {

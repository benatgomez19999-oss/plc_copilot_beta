import type { Machine } from '@plccopilot/pir';
...
export interface AlarmDiagnosticsOptions {
  /**
   * PIR `machines[…]` index this alarm collection belongs to. Defaults
   * to `0` because today `compileProject` only consumes the first
   * machine. The parameter exists so future multi-machine support can
   * pass the right index without re-touching this module.
   */
  machineIndex?: number;
}
 ...
 */
export function collectAlarmDiagnostics(
  machine: Machine,
  options: AlarmDiagnosticsOptions = {},
): Diagnostic[] {
  const out: Diagnostic[] = [];
  if (machine.alarms.length === 0) return out;
  ...
  machine.alarms.forEach((al, alarmIndex) => {
    if (!al.when) return;
    const ctx: ExpressionDiagnosticContext = {
      path: alarmWhenPath(machineIndex, alarmIndex),
      symbol: al.id,
      hint:
        'Fix the alarm condition expression before generating artifacts.',
    };
    ...
    const { diagnostics } = lowerExpression(
      al.when,
      table,
      new EdgeRegistry(),
      ctx,

import type { ProgramIR } from './program.js';
...
export interface SerializedProgram {
  project: { id: string; name: string; pirVersion: string };
  target: {
    vendor: string | null;
  ...
  blocks: Array<{ name: string; stationId: string | null }>;
  typeArtifacts: Array<{
    name: string;
    typeKind: 'equipment';
    fieldCount: number;
  ...
  dataBlocks: Array<{
    name: string;
    dbKind: 'params' | 'recipes' | 'alarms' | 'other';
    fieldCount: number;
  }>;
  tagTables: Array<{ name: string; kind: 'main'; rowCount: number }>;
  manifest: {
    generatedAt: string;
...
function toSerializable(program: ProgramIR): SerializedProgram {
  return {
    project: {
    ...
    diagnostics: program.diagnostics.map((d) => {
      const out: SerializedProgram['diagnostics'][number] = {
        code: d.code,
        severity: d.severity,
        message: d.message,
      };
      if (d.path !== undefined) out.path = d.path;
      if (d.stationId !== undefined) out.stationId = d.stationId;
      if (d.symbol !== undefined) out.symbol = d.symbol;

import type { Alarm, Parameter, Project } from '@plccopilot/pir';
import type { DataBlockArtifactIR, DataFieldIR } from './program.js';
import { CodegenError } from '../../types.js';

function alarmText(al: Alarm): string {
...
const PIR_TO_SIEMENS: Record<string, string> = {
  bool: 'Bool',
  int: 'Int',
  dint: 'DInt',
  real: 'Real',
...
function siemensType(pirDataType: string): string {
  return PIR_TO_SIEMENS[pirDataType] ?? 'Variant';
}

// ---------- DB_Alarms (v2: ack_all + set_/active_) ----------

export function buildDbAlarmsIR(
  project: Project,
): DataBlockArtifactIR | null {
  const machine = project.machines[0]!;
  if (machine.alarms.length === 0) return null;
...
function paramComment(p: Parameter): string {
  if (p.description) return sanitizeComment(p.description);
  const unit = p.unit ? ` [${p.unit}]` : '';
  return sanitizeComment(`${p.name}${unit}`);
}

function renderInitParam(p: Parameter): string {
...
export function buildDbParamsIR(
  project: Project,
): DataBlockArtifactIR | null {
  const machine = project.machines[0]!;
  if (machine.parameters.length === 0) return null;

  const sorted = machine.parameters
    .slice()
    .sort((a, b) => a.id.localeCompare(b.id));

  const fields: DataFieldIR[] = sorted.map((p) => ({
    name: p.id,
...
function renderInitRecipe(p: Parameter, value: number | boolean): string {
...
export function buildDbRecipesIR(
  project: Project,
): DataBlockArtifactIR | null {
  const machine = project.machines[0]!;
  if (machine.recipes.length === 0) return null;

  const paramsById = new Map<string, Parameter>(
    machine.parameters.map((p) => [p.id, p]),
  );
    ...
    for (const paramId of Object.keys(r.values).sort()) {
      ...
      if (!p) {
        ...
        throw new CodegenError(
          'UNKNOWN_PARAMETER',
          `Recipe "${r.id}" references unknown parameter "${paramId}".`,
          {
            path,
            symbol: paramId,
            hint:
              'Define the parameter in machine.parameters or remove it from the recipe.',
          },
        );
      }
      fields.push({

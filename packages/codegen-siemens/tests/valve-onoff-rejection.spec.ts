// Sprint 87A — `valve_onoff` is supported by CODESYS only in v0.
// Siemens / Rockwell façades MUST reject it via
// `READINESS_FAILED` so the per-target capability split stays
// load-bearing.
...
function valveProject(): Project {
  return {
    pir_version: '0.1.0',
    id: 'prj_valve87a_siemens_reject',
    name: 'Sprint 87A valve_onoff Siemens rejection',
    description: 'Synthetic project to assert Siemens still rejects valve_onoff.',
    machines: [
      {
        id: 'mch_v',
        name: 'Valve machine',
...
describe('generateSiemensProject — Sprint 87A rejects valve_onoff', () => {
  it('1. throws CodegenError(READINESS_FAILED) before compileProject runs', () => {
    ...
    try {
      generateSiemensProject(valveProject());
    } catch (e) {
    ...
    expect(caught?.message).toContain('siemens');

...
// ---------- Statement rendering ----------
export function renderStmt(stmt, level) {
    switch (stmt.kind) {
        case 'Assign': {
            const c = stmt.comment ? `  // ${stmt.comment}` : '';
            return [
                pad(level, `${renderRef(stmt.target, 'siemens')} := ${renderExpr(stmt.expr)};${c}`),
            ];
        }
        case 'Comment':
        ...
        case 'TonCall':
            return [
                pad(level, `${renderRef(stmt.instance, 'siemens')}(IN := ${renderExpr(stmt.inExpr)}, PT := T#${stmt.ptMs}MS);`),
            ];
        case 'FbCall': {
            ...
            const c = stmt.comment ? `  // ${stmt.comment}` : '';
            return [
                pad(level, `${renderRef(stmt.instance, 'siemens')}(${params});${c}`),
            ];
        }
    }
}
function renderIf(stmt, level) {
    const out = [];
    out.push(pad(level, `IF ${renderExpr(stmt.cond)} THEN`));
    for (const s of stmt.then)
        out.push(...renderStmt(s, level + 1));
    if (stmt.elseIfs) {
...
function renderCase(stmt, level) {
    const out = [];
...
// ---------- Variable sections ----------
export function renderVarSection(section, level) {
...
export function renderFunctionBlock(fb) {
    const lines = [];

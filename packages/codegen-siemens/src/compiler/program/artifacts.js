 ...
 */
export function renderProgramArtifacts(program) {
    const out = [];
    ...
    for (const a of out) {
        artifactBasenames.push(a.path.includes('/') ? a.path.slice(a.path.lastIndexOf('/') + 1) : a.path);
    }
...
function renderBlockArtifact(fb, program) {
    ...
    const stationDiags = fb.stationId
        ...
        : [];
    const artifact = { path, kind: 'scl', content };
 ...
 */
function renderManifestArtifact(m, artifactPaths) {
    const data = {

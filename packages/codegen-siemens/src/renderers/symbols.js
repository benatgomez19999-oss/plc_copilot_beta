...
export function buildSymbolContext(machine, station, path) {
    const io_by_id = new Map();
    for (const io of machine.io)
        io_by_id.set(io.id, io);
    const equipment_by_id = new Map();
    for (const s of machine.stations) {
        for (const e of s.equipment)
            equipment_by_id.set(e.id, e);
    }
    return {
        machine,
        station,
...
export function renderKeyword(kw) {
    switch (kw) {
        case 'mode':
            return '#i_mode';
        case 'start_cmd':
        ...
        default:
            ...
            throw new CodegenError('UNKNOWN_KEYWORD', `Unknown keyword "${kw}".`, {
                symbol: kw,
                hint: `Use one of the supported keywords: ${KNOWN_KEYWORDS.join(', ')}.`,
            });
    }
}
export function resolveToSclSymbol(ref, ctx) {
    if (EXPR_KEYWORDS.has(ref))
        return renderKeyword(ref);
    if (ref.includes('.')) {
        ...
        if (!parsed) {
            ...
            throw new CodegenError('INVALID_REF', `Reference "${ref}" has an invalid format.`, {
                path: ctx.path,
        ...
        if (!eq) {
            throw new CodegenError('UNKNOWN_EQUIPMENT', `Reference "${ref}" points to unknown equipment "${parsed.equipment_id}".`, {
                path: ctx.path,
                stationId: ctx.station.id,
        ...
        if (!ioId) {
            throw new CodegenError('UNBOUND_ROLE', `Equipment "${parsed.equipment_id}" has no IO binding for role "${parsed.role}".`, {
                path: ctx.path,
                stationId: ctx.station.id,
        ...
        if (!io) {
            throw new CodegenError('UNKNOWN_IO', `IO "${ioId}" bound to "${ref}" is not declared in machine.io.`, {
                path: ctx.path,
                stationId: ctx.station.id,
                symbol: ioId,
    ...
    if (io)
        return tagSymbol(io.id);
    if (ctx.parameter_ids.has(ref))
        return tagSymbol(ref);
    const eq = ctx.equipment_by_id.get(ref);
    if (eq)
        return tagSymbol(eq.id);
    throw new CodegenError('UNRESOLVED_REF', `Reference "${ref}" does not resolve to any IO, parameter, equipment, or keyword.`, {
        path: ctx.path,
        stationId: ctx.station.id,
        symbol: ref,
...
export function ioSymbolForRole(eq, role, ctx) {
    const ioId = eq.io_bindings[role];
    if (!ioId) {
        throw new CodegenError('UNBOUND_ROLE', `Equipment "${eq.id}" has no IO binding for role "${role}".`, {
            path: ctx.path,
            stationId: ctx.station.id,
            symbol: `${eq.id}.${role}`,
    ...
    const io = ctx.io_by_id.get(ioId);
    if (!io) {
        throw new CodegenError('UNKNOWN_IO', `IO "${ioId}" bound to "${eq.id}.${role}" is not declared in machine.io.`, {
            path: ctx.path,
            stationId: ctx.station.id,
            symbol: ioId,
            hint: 'Add the IO signal to machine.io, or fix the binding to reference an existing IO id.',
...
export function tagSymbol(id) {
    return `"${id}"`;
}
export function alarmSymbol(alarmId) {
    return `"${alarmId}"`;
}
export function localSymbol(name) {
    return `#${name}`;
}

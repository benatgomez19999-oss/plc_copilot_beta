import { analyzeExpression } from '@plccopilot/pir';
import { CodegenError } from '../types.js';
import { sanitizeSymbol } from '../naming.js';
import { localSymbol, renderKeyword, resolveToSclSymbol, } from './symbols.js';
export function renderExpression(expr, ctx) {
    const a = analyzeExpression(expr);
    if (!a.ok) {
        throw new CodegenError('INVALID_EXPR', `Cannot render invalid expression "${expr}": ${a.issues.join('; ')}`, {
            path: ctx.path,
...
function consumeFunctionCall(tokens, start, ctx) {
...
function renderFunctionCall(name, args, ctx) {
    if (args.length === 0) {
        throw new CodegenError('INVALID_FN', `Function "${name}" requires at least one argument.`, {
            path: ctx.path,
            stationId: ctx.station.id,
            symbol: name,
    ...
    switch (name) {
        ...
        default:
            throw new CodegenError('UNKNOWN_FN', `Function "${name}" is not supported by the v0.1 Siemens generator.`, {
                path: ctx.path,
                stationId: ctx.station.id,
                symbol: name,

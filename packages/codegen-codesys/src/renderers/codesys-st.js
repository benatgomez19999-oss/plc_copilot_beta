 ...
 *   #name          → name
 */
export function siemensToCodesysText(s) {
    let r = s;
...
export function renderExprCodesys(e) {
    switch (e.kind) {
        case 'Raw':
...
// ---------- Statement rendering ----------
export function renderStmtCodesys(stmt, level) {
    switch (stmt.kind) {
        case 'Assign': {
            const c = stmt.comment ? ` (* ${stmt.comment} *)` : '';
            return [
                pad(level, `${renderRef(stmt.target)} := ${renderExprCodesys(stmt.expr)};${c}`),
            ];
        }
        case 'Comment':
        ...
        case 'TonCall':
            return [
                pad(level, `${renderRef(stmt.instance)}(IN := ${renderExprCodesys(stmt.inExpr)}, PT := T#${stmt.ptMs}MS);`),
            ];
        case 'FbCall': {
...
function renderCaseCodesys(stmt, level) {
    const out = [];
    ...
    for (const arm of stmt.arms) {
        ...
        if (arm.body.length === 0) {
            ...
            for (const s of arm.body)
                out.push(...renderStmtCodesys(s, level + 2));
        }
    }
    out.push(pad(level, `ELSE`));
...
// ---------- VAR sections ----------
export function renderVarSectionCodesys(section, level) {
...
// ---------- FUNCTION_BLOCK ----------
export function renderFunctionBlockCodesys(fb) {
    const lines = [];
    ...
    for (const s of fb.body)
        lines.push(...renderStmtCodesys(s, 0));

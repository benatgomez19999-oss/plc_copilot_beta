...
export function tokenize(src: string): { tokens: Token[]; issues: string[] } {
  const tokens: Token[] = [];
  const issues: string[] = [];
  let i = 0;
  ...
  while (i < src.length) {
    ...
    if (c === '.') { tokens.push({ type: 'dot', value: '.', start: i, end: i + 1 }); i++; continue; }

    // 2-char operators
    const c2 = src[i + 1];
    if (c === '&' && c2 === '&') { tokens.push({ type: 'and', value: '&&', start: i, end: i + 2 }); i += 2; continue; }
    if (c === '|' && c2 === '|') { tokens.push({ type: 'or', value: '||', start: i, end: i + 2 }); i += 2; continue; }
    if (c === '=' && c2 === '=') { tokens.push({ type: 'eq', value: '==', start: i, end: i + 2 }); i += 2; continue; }
    if (c === '!' && c2 === '=') { tokens.push({ type: 'neq', value: '!=', start: i, end: i + 2 }); i += 2; continue; }
    if (c === '<' && c2 === '=') { tokens.push({ type: 'lte', value: '<=', start: i, end: i + 2 }); i += 2; continue; }
    if (c === '>' && c2 === '=') { tokens.push({ type: 'gte', value: '>=', start: i, end: i + 2 }); i += 2; continue; }

    // single-char operators
    if (c === '!') { tokens.push({ type: 'not', value: '!', start: i, end: i + 1 }); i++; continue; }
    if (c === '<') { tokens.push({ type: 'lt', value: '<', start: i, end: i + 1 }); i++; continue; }
    if (c === '>') { tokens.push({ type: 'gt', value: '>', start: i, end: i + 1 }); i++; continue; }
    ...
    if (ALPHA.test(c)) {
      const start = i;
      while (i < src.length && ALNUM.test(src[i]!)) i++;
      const value = src.slice(start, i);
      const type: TokenType = EXPR_KEYWORDS.has(value) ? 'keyword' : 'ident';
 ...
 * (per v0.1 grammar).
 */
export function analyzeExpression(expr: string): AnalyzeResult {
  ...
  while (i < tokens.length) {
    ...
    if (tk.type === 'ident' && EXPR_FUNCTIONS.has(tk.value)) {
      ...
      if (!next || next.type !== 'lparen') {
        issues.push(
          `function name "${tk.value}" used as identifier at position ${tk.start}`,
        );
        i++;
        continue;
      }
    ...
    if (tk.type === 'ident') {
      ...
      if (next && next.type === 'lparen') {
        ...
        if (i < tokens.length) i++; // past closing ')'
        continue;
      }

...
const SPEED_SETPOINT_HZ_ALIASES: ReadonlySet<string> = new Set([
  'hz',
  'hertz',
]);

export function normalizeSpeedSetpointUnit(
  unit: string | undefined,
): 'Hz' | null {
  if (typeof unit !== 'string') return null;
  const trimmed = unit.trim();
  if (trimmed.length === 0) return null;
  if (SPEED_SETPOINT_HZ_ALIASES.has(trimmed.toLowerCase())) return 'Hz';
  return null;
}
...
export function runParameterRules(
  ...
  ctx.machine.stations.forEach((station) => {
    station.equipment.forEach((eq) => {
      if (eq.type !== 'motor_vfd_simple') return;
      ...
      if (!param) return; // R-EQ-05 already flags missing parameter.

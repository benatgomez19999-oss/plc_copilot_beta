...
function checkIdShape(report, path, id) {
    if (!ID_RE.test(id)) {
        addIssue(report, {
            rule: 'R-ID-01',
            severity: 'error',
...
function uniq(report, rule, entityName, items, pathOf) {
    const seen = new Map();
...
export function runIdRules(project, ctx, report) {
    const m = ctx.machine;
    ...
    m.stations.forEach((station, sIdx) => {
        ...
        uniq(report, 'R-ID-04', 'state', station.sequence.states, (i) => `${sPath}.sequence.states[${i}]`);
        uniq(report, 'R-SM-09', 'transition', station.sequence.transitions, (i) => `${sPath}.sequence.transitions[${i}]`);
    ...
    m.safety_groups.forEach((g, i) => checkIdShape(report, `${base}.safety_groups[${i}].id`, g.id));
}

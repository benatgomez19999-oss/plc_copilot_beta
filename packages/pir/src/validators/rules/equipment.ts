...
interface RoleSpec {
  ...
  // 'bool' = strictly bool; 'numeric' = int|dint|real (not bool).
  dataClass?: 'bool' | 'numeric';
}
 ...
 */
function roleSpec(eqType: EquipmentType, role: string): RoleSpec {
  ...
  if (role.endsWith('_out')) return { direction: 'out', dataClass: 'bool' };
  if (role.endsWith('_in')) return { direction: 'in', dataClass: 'bool' };
  if (role.startsWith('sensor_')) return { direction: 'in', dataClass: 'bool' };
  if (role.endsWith('_fb')) return { direction: 'in', dataClass: 'bool' };
  return {};
}

function dtypeCompatible(expected: 'bool' | 'numeric', actual: SignalDataType): boolean {
  if (expected === 'bool') return actual === 'bool';
  return actual !== 'bool';
}

export function runEquipmentRules(
  _project: Project,
  ...
  ctx.machine.stations.forEach((station, sIdx) => {
    station.equipment.forEach((eq, eIdx) => {
      ...
      const ioToRoles = new Map<string, string[]>();
      for (const [role, ioId] of Object.entries(eq.io_bindings)) {
        ...
        const isKnownRole =
          shape.required_io.includes(role) || shape.optional_io.includes(role);
        if (!isKnownRole) {
          addIssue(report, {
            rule: 'R-EQ-02',
            severity: 'error',
      ...
      for (const [ioId, roles] of ioToRoles) {
        ...
        const dirs = new Set(
          specs.map((s) => s.direction).filter((d): d is SignalDirection => !!d),
        );
        const classes = new Set(
          specs
            .map((s) => s.dataClass)
            .filter((d): d is 'bool' | 'numeric' => !!d),
        );
      ...
      //
      // Two sub-rules:
      //   (A) Every REQUIRED numeric-output role must be bound to a
      //       parameter source (otherwise the lowering would have to
      //       synthesise a value or leave the IO dangling).
      //   (B) Every entry actually present in `io_setpoint_bindings`
      //       must reference a known role of the right shape (numeric
      //       output, also bound in `io_bindings`) and a numeric
      //       parameter that exists in this machine.
      ...
      for (const [role, paramId] of Object.entries(setpointBindings)) {
        ...
        const isKnownRole =
          shape.required_io.includes(role) ||

import type { Project, Transition } from '../../domain/types.js';
import { addIssue, type ValidationReport } from '../report.js';
import type { ValidationContext } from '../context.js';

export function runSequenceRules(
  _project: Project,
  ...
  ctx.machine.stations.forEach((station, sIdx) => {
    ...
    if (initial !== undefined) {
      ...
      states.forEach((st, stIdx) => {
        if (st.id === initial.id) return;
        if (!reachable.has(st.id)) {
          const reachedByWildcard = transitions.some(
            (t) => t.from === '*' && t.to === st.id,
          );
          addIssue(report, {
            rule: 'R-SM-03',
    ...
    for (const [from, list] of byFrom) {
      const seen = new Map<number, Transition>();
      for (const t of list) {

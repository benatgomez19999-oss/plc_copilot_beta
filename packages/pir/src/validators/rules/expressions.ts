...
interface ExprSource {
  expr: string;
  path: string;
}

function collectExpressionSources(ctx: ValidationContext): ExprSource[] {
  ...
  ctx.machine.stations.forEach((station, sIdx) => {
    station.sequence.transitions.forEach((t, tIdx) => {
      ...
      if (t.trigger) sources.push({ expr: t.trigger, path: `${tp}.trigger` });
      if (t.guard) sources.push({ expr: t.guard, path: `${tp}.guard` });
    });
  });
...
export function runExpressionRules(
  _project: Project,
  ctx: ValidationContext,
  report: ValidationReport,
): void {
  const ioIds = new Set<string>(ctx.machine.io.map((io) => io.id));
  const parameterIds = new Set<string>(ctx.machine.parameters.map((p) => p.id));

  const equipmentShapes = new Map<string, ShapeRoles>();

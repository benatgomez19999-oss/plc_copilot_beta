export function rawAddress(signal) {
    const a = signal.address;
    const base = a.memory_area === 'DB'
...
export function buildContext(project) {
    ...
    for (const s of machine.stations) {
        if (!ctx.stations_by_id.has(s.id))
            ctx.stations_by_id.set(s.id, s);
        for (const e of s.equipment) {
            if (!ctx.equipment_by_id.has(e.id))
                ctx.equipment_by_id.set(e.id, e);
        }
        ctx.sequences.push({ station: s, sequence: s.sequence });
    }
    for (const io of machine.io) {
        if (!ctx.io_by_id.has(io.id))
            ctx.io_by_id.set(io.id, io);
        const raw = rawAddress(io);
        const list = ctx.io_by_raw_address.get(raw);
        if (list)
            list.push(io);
        else
            ctx.io_by_raw_address.set(raw, [io]);
    }
    for (const a of machine.alarms) {
        if (!ctx.alarms_by_id.has(a.id))
            ctx.alarms_by_id.set(a.id, a);
    }
    for (const i of machine.interlocks) {
        if (!ctx.interlocks_by_id.has(i.id))
            ctx.interlocks_by_id.set(i.id, i);
    }
    for (const p of machine.parameters) {
        if (!ctx.parameters_by_id.has(p.id))
            ctx.parameters_by_id.set(p.id, p);
    }
    for (const r of machine.recipes) {
        if (!ctx.recipes_by_id.has(r.id))
            ctx.recipes_by_id.set(r.id, r);
    }
...
export function machinePath(_context) {
    return `$.machines[0]`;
}

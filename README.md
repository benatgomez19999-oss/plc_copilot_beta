...
| `pnpm release:provenance --config-only [--version X.Y.Z] [--json]` | _(Sprint 65 stub, kept verbatim)_ Local-only check — confirms the publish workflow grants `id-token: write` + references `--provenance`, and that the publish command builder still hardcodes `--provenance` for every supported tag and never `--dry-run`. No network. Safe for `ci:contracts`-style invocation |
| `pnpm release:provenance --metadata-only --version X.Y.Z [--registry URL] [--json]` | _(Sprint 70)_ Read-only registry verification: `npm view` per package, fetches the npm attestations endpoint, decodes the DSSE in-toto Statement, validates the SLSA-provenance claims (subject = `pkg:npm/@plccopilot/<pkg>@X.Y.Z`, `predicate.workflow.repository` = expected GitHub repo, `predicate.workflow.path` = `.github/workflows/publish.yml`). NEVER mutates npm. Cryptographic Sigstore verification is **not implemented** |
| `pnpm release:provenance --version X.Y.Z [--registry URL] [--json]` | _(Sprint 70 default)_ Config + metadata + claims. Falls back to config-only if `--version` cannot be inferred from the workspace |
| `pnpm release:audit-signatures --version X.Y.Z [--registry URL] [--package @plccopilot/<pkg>] [--json] [--keep]` | _(Sprint 71)_ Wraps `npm audit signatures`: installs the package into a fresh `os.tmpdir()` consumer project, runs `npm audit signatures --json`, asserts `invalid==[]` AND `missing==[]`. npm-supported cryptographic check — verifies registry signatures + provenance attestations. NEVER mutates npm. Default `--package` is `@plccopilot/cli` (pulls full internal graph) |
